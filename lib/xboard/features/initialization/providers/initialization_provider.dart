...
class XBoardInitializationNotifier extends StateNotifier<InitializationState> {
  ...
  Future<void> initialize() async {
    ...
    try {
      ...
      final isWyxV2Board = await ref.read(isWyxV2BoardBackendProvider.future);
      if (isWyxV2Board) {
        await _initializeWyxV2Board();
        return;
      }
  ...
  Future<void> _initializeWyxV2Board() async {
    _logger.info('[Initialization] 🔐 初始化 wyx_v2board 安全链路');
    state = state.copyWith(
      status: InitializationStatus.initializingSDK,
      errorMessage: null,
      currentStepDescription: '正在初始化安全连接...',
    );

    await ref.read(wyxV2BoardAdapterProvider.future);
    ...
    _logger.info('[Initialization] ✅ wyx_v2board 安全链路初始化完成');
  }
