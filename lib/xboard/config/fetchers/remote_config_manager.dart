...
/// 普通 JSON 配置源实现。
///
/// 用于本地调试或其它无需解密的远程 JSON 源，保留调用方配置的 sourceName。
class PlainJsonConfigSource implements ConfigSource {
  final IHttpClient _httpClient;
  final String name;
  final String url;
  final Duration timeout;
  ...
  Future<ConfigResult<Map<String, dynamic>>> fetchConfig() async {
    try {
      _logger.info('开始获取配置源 [$sourceName]: $url');
      final rawData = await _httpClient.getString(url, timeout: timeout);

      if (rawData == null || rawData.trim().isEmpty) {
        _logger.error('配置源 [$sourceName] 获取失败: 数据为空');
        return ConfigResult.failure('配置源 [$sourceName] 获取失败', sourceName);
      }

      final jsonData = json.decode(rawData.trim()) as Map<String, dynamic>;
      _logger.info('配置源 [$sourceName] 获取成功');
      return ConfigResult.success(jsonData, sourceName);
    } catch (e) {
      _logger.error('配置源 [$sourceName] 异常', e);
      return ConfigResult.failure(
        '配置源 [$sourceName] 异常: ${e.toString()}',
        sourceName,
      );
    }
  }
