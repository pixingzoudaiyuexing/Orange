...
class ConfigValidator {
    ...
    if (!json.containsKey('panels') &&
      ...
      errors.add(
        ValidationError(
          field: 'root',
          message:
              'Configuration must contain at least one of: panels, proxy, ws, update, subscription',
  ...
  List<ValidationError> _validatePanelStructure(dynamic panels) {
    ...
    panelsMap.forEach((provider, panelList) {
      ...
      for (int i = 0; i < list.length; i++) {
        ...
        // online_support 是可选展示能力。解析阶段会只采纳合法的 Crisp 新结构；
        // 非新结构或缺失字段时视为未配置客服，不阻断 RemoteConfig 加载。
      }
    });
  ...
  /// 验证服务列表
  List<ValidationError> _validateServiceList(
    dynamic serviceList,
    String serviceType,
  ) {
    final errors = <ValidationError>[];
    ...
    for (int i = 0; i < list.length; i++) {
      ...
      if (serviceType == 'proxy' &&
          (!itemMap.containsKey('protocol') ||
              itemMap['protocol'] is! String)) {
        errors.add(
          ValidationError(
...
class ConfigurationParser {
  ...
  /// 从远程配置结果中提取配置数据
  Map<String, dynamic>? extractConfigFromRemoteResult(
  ...
  Map<String, dynamic> _mergeConfigurations(
    ...
    for (final config in configs) {
      ...
      if (config.containsKey('panels')) {
        ...
        final existingPanels =
            (merged['panels'] as Map<String, dynamic>?) ?? {};
      ...
      if (config.containsKey('subscription')) {
        final existingSubscription =
            (merged['subscription'] as Map<String, dynamic>?) ?? {};
